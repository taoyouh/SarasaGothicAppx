...
Update-ApplicationSubmission -AppId $appId -SubmissionDataPath (Join-Path $submissionPackageName "${submissionPackageName}.json") -PackagePath (Join-Path $outPath ($submissionPackageName + ".zip")) -Force -ReplacePackages -UpdateListings -TargetPublishMode Manual -AutoCommit
